...
public sealed class CseManager
{
    ...
    {
        _nonDeterministicFunctions = nonDeterministicFunctions ??
                                     throw new ArgumentNullException(nameof(nonDeterministicFunctions));
    }
    ...
    public void Initialize(Node queryNode, bool enabled)
    {
        ...
        foreach (var kvp in analyzer.GetCacheSlotMap()) _slotMap[kvp.Key] = kvp.Value;
    }
        ...
        bool isInsideCaseWhen)
    {
        ...
        if (!_isEnabled || !_slotMap.TryGetValue(expressionId, out _)) return expression;
        ...
        if (!_computedInCurrentRow.Add(expressionId)) return variableIdentifier;
