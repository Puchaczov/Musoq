...
internal sealed class SourcePredicateMetadataPass : IPlanOptimizationPass<PhysicalNode>
{
    ...
    private static IrExpression[] ResolvePredicates(PhysicalSchemaScanNode scan, PhysicalOptimizationState state)
    {
        return !string.IsNullOrWhiteSpace(scan.SourceContextId) &&
               state.Properties.PushedPredicatesBySourceId.TryGetValue(scan.SourceContextId, out var predicates)
            ? predicates
            : [];
    }
