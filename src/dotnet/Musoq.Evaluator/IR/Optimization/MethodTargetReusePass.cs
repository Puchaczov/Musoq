...
internal sealed class MethodTargetReusePass : IPlanOptimizationPass<ExecutionPlan>
{
    ...
    private sealed class MethodTargetReuseRewriter(string planIdentifier) : ExecutionIrRewriter
    {
        ...
        {
            var rewritten = (ExecutionStrictCast)base.RewriteStrictCast(expression);
            if (rewritten.Target != null || !StrictCastLibraryConversionFacts.NeedsLibraryTarget(rewritten.Expression.ReturnType, rewritten.ReturnType))
                return rewritten;
            var target = CurrentRegistry.GetOrAdd(typeof(Musoq.Plugins.LibraryBase), _preferredTargetNamePrefix);
            if (target == null)
                return rewritten;
            AssignedTargets++;
            return rewritten with { Target = target };
        }
