...
internal sealed class JoinStrategySelectionPass : IPlanOptimizationPass<PhysicalNode>
{
        ...
        PhysicalOptimizationState state)
    {
        ...
        var strategy = PhysicalStrategyRules.ChooseJoinStrategy(
            ...
            state.Properties.CardinalityFacts);
        ...
        if (strategyKind == JoinStrategyKind.NestedLoop)
            state.AddDecision(NestedLoopJoinRiskAdvisor.CreateRiskDecision(
                join.Kind, left, right, state.Properties.CardinalityFacts));
        ...
        PhysicalOptimizationState state)
    {
        ...
        if (source is PhysicalSchemaScanNode scan &&
            state.ShapeResolver?.ResolveSourceShape(scan) is { } shape)
        {
            return !CanUseRangeProbeSource(shape, RowShapeLookup.ResolveSourceRuntimeType(shape));
        }
    ...
    private static bool IsDynamicFlatJoinInput(PhysicalNode node, PhysicalOptimizationState state)
    {
        return UnwrapFilterInput(node) is PhysicalSchemaScanNode scan &&
               state.ShapeResolver?.ResolveSourceShape(scan) is ExpandoAdapterShape;
    }
    ...
    private static bool CanUseRangeProbeSource(RowShape sourceShape, Type rowType)
    {
