...
public sealed partial class PhysicalToExecutionPlanBuilder
{
        ...
        ExecutionBlock matchedBody)
    {
        ...
        if (!TryCollectRequiredPayloadFieldNames(matchedBody, context.Sides.Build.Variable.Name, out var requiredFields))
            return context;
        ...
        var prunedPayload = new FusedHashPayload(prunedShape, prunedValues);
        var build = context.Sides.Build with
        {
            FusedHashPayload = prunedPayload,
            Shapes = ReplacePayloadShape(context.Sides.Build.Shapes, payload.Shape, prunedShape)
        };
