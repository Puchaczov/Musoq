...
public sealed partial class PhysicalToExecutionPlanBuilder
{
        ...
        ExecutionVariable windowIndex)
    {
        ...
        var aggregateRead = ResolveWindowAggregateSourceRead(field.Expression, sourceLookup);
        if (aggregateRead != null)
            return BuildResult<ExecutionExpression>.Success(aggregateRead);

        var expression = ConvertWindowExpression(field.Expression, sourceLookup, windowResults, windowIndex);
        ...
        ExecutionVariable windowIndex)
    {
        switch (expression)
        {
            ...
            case AggregateRef:
                var aggregateRead = ResolveWindowAggregateSourceRead(expression, sourceLookup);
            ...
            case BinaryOp binary:
                var left = ConvertWindowExpression(binary.Left, sourceLookup, windowResults, windowIndex);
                if (!left.Supported)
                    return BuildResult<ExecutionExpression>.Unsupported(left.UnsupportedReason);

                var right = ConvertWindowExpression(binary.Right, sourceLookup, windowResults, windowIndex);
                if (!right.Supported)
            ...
            case UnaryOp unary:
                var operand = ConvertWindowExpression(unary.Operand, sourceLookup, windowResults, windowIndex);
            ...
            case ArrayAccess arrayAccess:
                return ConvertWindowArrayAccess(arrayAccess, sourceLookup, windowResults, windowIndex);
            case IsNullCheck isNull:
                return ConvertWindowIsNullCheck(isNull, sourceLookup, windowResults, windowIndex);
            case InCheck inCheck:
                return ConvertWindowInCheck(inCheck, sourceLookup, windowResults, windowIndex);
            case CollectionInCheck collectionInCheck: return ConvertWindowCollectionInCheck(collectionInCheck, sourceLookup, windowResults, windowIndex);
            case PatternMatch patternMatch:
                return ConvertWindowPatternMatch(patternMatch, sourceLookup, windowResults, windowIndex);
            case Between between:
                return ConvertWindowBetween(between, sourceLookup, windowResults, windowIndex);
            case CaseWhen caseWhen:
                return ConvertWindowCaseWhen(caseWhen, sourceLookup, windowResults, windowIndex);
            case Coalesce coalesce:
                return ConvertWindowCoalesce(coalesce, sourceLookup, windowResults, windowIndex);
            case MethodCall methodCall:
                return ConvertWindowMethodCall(methodCall, sourceLookup, windowResults, windowIndex);
            case StrictCast strictCast:
                return ConvertWindowStrictCast(strictCast, sourceLookup, windowResults, windowIndex);
            default:
    ...
    private static BuildResult<ExecutionExpression> ConvertWindowArrayAccess(
        ...
        ExecutionVariable windowIndex)
    {
        var array = ConvertWindowExpression(arrayAccess.Array, sourceLookup, windowResults, windowIndex);
        if (!array.Supported)
            return array;

        var index = ConvertWindowExpression(arrayAccess.Index, sourceLookup, windowResults, windowIndex);
        if (!index.Supported)
            return index;
    ...
    private static BuildResult<ExecutionExpression> ConvertWindowIsNullCheck(
        ...
        ExecutionVariable windowIndex)
    {
        var expression = ConvertWindowExpression(isNull.Expression, sourceLookup, windowResults, windowIndex);
        return expression.Supported
    ...
    private static BuildResult<ExecutionExpression> ConvertWindowInCheck(
        ...
        ExecutionVariable windowIndex)
    {
        var expression = ConvertWindowExpression(inCheck.Expression, sourceLookup, windowResults, windowIndex);
        if (!expression.Supported)
            return expression;

        var values = ConvertWindowExpressions(inCheck.Values, sourceLookup, windowResults, windowIndex);
    ...
    private static BuildResult<ExecutionExpression> ConvertWindowPatternMatch(
        ...
        ExecutionVariable windowIndex)
    {
        var expression = ConvertWindowExpression(patternMatch.Expression, sourceLookup, windowResults, windowIndex);
        if (!expression.Supported)
            return expression;

        var pattern = ConvertWindowExpression(patternMatch.Pattern, sourceLookup, windowResults, windowIndex);
    ...
    private static BuildResult<ExecutionExpression> ConvertWindowBetween(
        ...
        ExecutionVariable windowIndex)
    {
        var expression = ConvertWindowExpression(between.Expression, sourceLookup, windowResults, windowIndex);
        if (!expression.Supported)
            return expression;

        var low = ConvertWindowExpression(between.Low, sourceLookup, windowResults, windowIndex);
        if (!low.Supported)
            return low;

        var high = ConvertWindowExpression(between.High, sourceLookup, windowResults, windowIndex);
        ...
        ExecutionVariable windowIndex)
    {
        ...
        {
            var condition = ConvertWindowExpression(branch.Condition, sourceLookup, windowResults, windowIndex);
            if (!condition.Supported)
                return condition;

            var result = ConvertWindowExpression(branch.Result, sourceLookup, windowResults, windowIndex);
            if (!result.Supported)
                return result;
        ...
        var elseExpression = caseWhen.ElseExpression == null
            ? null
            : ConvertWindowExpression(caseWhen.ElseExpression, sourceLookup, windowResults, windowIndex);
    ...
    private static BuildResult<ExecutionExpression> ConvertWindowCoalesce(
        ...
        ExecutionVariable windowIndex)
    {
        var expressions = ConvertWindowExpressions(coalesce.Expressions, sourceLookup, windowResults, windowIndex);
        ...
        ExecutionVariable windowIndex)
    {
        ...
        {
            var result = ConvertWindowExpression(expression, sourceLookup, windowResults, windowIndex);
            if (!result.Supported)
    ...
    private static BuildResult<ExecutionExpression> ConvertWindowMethodCall(
        ...
        ExecutionVariable windowIndex)
    {
        var aggregateRead = ResolveWindowAggregateSourceRead(methodCall, sourceLookup);
        if (aggregateRead != null)
            return BuildResult<ExecutionExpression>.Success(aggregateRead);
        ...
        {
            var converted = ConvertWindowExpression(argument, sourceLookup, windowResults, windowIndex);
    ...
    private static BuildResult<ExecutionExpression> ConvertWindowStrictCast(
        ...
        ExecutionVariable windowIndex)
    {
        var expression = ConvertWindowExpression(strictCast.Expression, sourceLookup, windowResults, windowIndex);
        return expression.Supported
