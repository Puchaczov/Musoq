...
public sealed partial class ExecutionCSharpRenderer
{
        ...
        string partitionCountName)
    {
        ...
        var valueRead = $"{value}{(Nullable.GetUnderlyingType(kernel.Descriptor.InputType) != null ? ".Value" : string.Empty)}";
        var valuePresent = Nullable.GetUnderlyingType(kernel.Descriptor.InputType) != null
            ? $"{value}.HasValue"
            : "true";
        ...
        var itemDeclarations = CreateWindowAggregateBoundedMinMaxItemDeclarations(
            ...
            frameValueIndex,
            12);
        var resultType = EvaluationHelper.GetCastableType(kernel.Descriptor.ResultType);
        var source =
            ...
            $"    var {frameStart} = {CreateWindowAggregateFrameStartExpression(frame.Start, partitionIndex, partitionCountName).NormalizeWhitespace().ToFullString()};" + Environment.NewLine +
            $"    var {frameEnd} = {CreateWindowAggregateFrameEndExpression(frame.End, partitionIndex, partitionCountName).NormalizeWhitespace().ToFullString()};" + Environment.NewLine +
