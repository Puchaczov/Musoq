...
public sealed partial class ExecutionCSharpRenderer
{
    ...
    private IEnumerable<StatementSyntax> RenderParallelBlock(ExecutionParallelBlock parallel, ExecutionRenderContext context)
    {
        ...
        yield return CreateLocalDeclaration(
            ...
            SyntaxFactory.ObjectCreationExpression(SyntaxFactory.IdentifierName(CreateParallelRunnerTypeName(parallel)))
                .WithArgumentList(CreateArgumentList(CreateParallelRunnerConstructorArguments(parallel, captures))));
    ...
    private IEnumerable<MemberDeclarationSyntax> CreateParallelBlockMembers(ExecutionParallelBlock parallel)
    {
        var captures = CollectParallelBlockCaptures(parallel);

        for (var index = 0; index < parallel.Tasks.Count; index++)
            yield return CreateParallelTaskBuildFunction(parallel, index, captures);

        yield return CreateParallelRunnerClass(parallel, captures);
    }
        ...
        IReadOnlyList<CapturedLocal> captures)
    {
        ...
        var statements = CreateParallelTaskBuildFunctionStatements(task);
        ...
        return SyntaxFactory.MethodDeclaration(
            ...
            .WithParameterList(CreateParallelTaskParameterList(parallel, captures))
    ...
    private List<StatementSyntax> CreateParallelTaskBuildFunctionStatements(ExecutionParallelTask task)
    {
        ...
        try
        {
            ...
            statements.AddRange(RenderParallelTaskBody(task).Statements);
        ...
        IReadOnlyList<CapturedLocal> captures)
    {
        ...
        members.AddRange(CreateParallelRunnerFields(parallel, captures));
        members.AddRange(parallel.Tasks.Select(CreateParallelTaskResultProperty));
        members.Add(CreateParallelRunnerConstructor(parallel, captures));
        members.AddRange(parallel.Tasks.Select((_, index) => CreateParallelTaskRunMethod(parallel, index, captures)));
    ...
    private IEnumerable<FieldDeclarationSyntax> CreateParallelRunnerFields(
        ...
        IReadOnlyList<CapturedLocal> captures)
    {
        foreach (var member in CreateParallelRunnerRuntimeMembers(parallel))
    ...
    private ConstructorDeclarationSyntax CreateParallelRunnerConstructor(
        ...
        IReadOnlyList<CapturedLocal> captures)
    {
        var qualifyShadowedRuntimeMembers = NeedsParallelTaskPhaseChanged(parallel);
        var assignments = CreateParallelRunnerRuntimeMembers(parallel)
            .Select<ParallelRunnerRuntimeMember, StatementSyntax>(member =>
                SyntaxFactory.ExpressionStatement(SyntaxFactory.AssignmentExpression(
                    SyntaxKind.SimpleAssignmentExpression,
                    CreateParallelRunnerAssignmentTarget(member, qualifyShadowedRuntimeMembers),
        ...
        return SyntaxFactory.ConstructorDeclaration(CreateParallelRunnerTypeName(parallel))
            ...
            .WithParameterList(SyntaxFactory.ParameterList(
                SyntaxFactory.SeparatedList(CreateParallelRunnerConstructorParameters(parallel, captures))))
            .WithBody(StatementEmitter.CreateBlock(assignments));
    }

    private static ExpressionSyntax CreateParallelRunnerAssignmentTarget(
        ParallelRunnerRuntimeMember member,
        bool qualifyShadowedRuntimeMembers)
    {
        return qualifyShadowedRuntimeMembers &&
               string.Equals(member.ParameterName, member.FieldName, StringComparison.Ordinal)
        ...
        IReadOnlyList<CapturedLocal> captures)
    {
        var assignment = SyntaxFactory.ExpressionStatement(SyntaxFactory.AssignmentExpression(
            ...
            SyntaxFactory.InvocationExpression(SyntaxFactory.IdentifierName(
                    CreateParallelTaskBuildFunctionName(parallel, taskIndex)))
                .WithArgumentList(CreateArgumentList(CreateParallelRunnerStaticTaskArguments(parallel, captures)))));
    ...
    private CapturedLocal[] CollectParallelBlockCaptures(ExecutionParallelBlock parallel)
    {
        var excludedNames = new HashSet<string>(CreateRuntimeHelperParameterNames(), StringComparer.Ordinal);

        foreach (var runtimeMember in CreateParallelRunnerRuntimeMembers(parallel))
    ...
    private BlockSyntax RenderParallelTaskBody(ExecutionParallelTask task)
    {
        ...
        try
        {
            ...
            statements.AddRange(RenderBlock(task.Body).Statements);
