...
internal sealed partial class PhysicalLoweringImplementation
{
        ...
        out LoweringScope updatedScope)
    {
        ...
        var buildState = new SidecarBuildState(scope.CteSidecarHashPayloads, []);
        foreach (var spec in specs.OrderBy(static spec => spec.IndexSlot))
        {
            var next = CreateCteSidecarIndexBuild(
                result.Table.Name,
                result.RowShape,
                spec,
                canUseHashPayloads,
                buildState.Payloads);
            buildState = new SidecarBuildState(
                next.Payloads,
                [..buildState.Builds, ..next.Builds]);
        }

        updatedScope = scope.WithCteSidecarHashPayloads(buildState.Payloads);
        var builds = buildState.Builds.ToArray();
