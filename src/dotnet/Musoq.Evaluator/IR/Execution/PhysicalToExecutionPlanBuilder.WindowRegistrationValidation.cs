...
{
    private static BuildResult<IReadOnlyList<WindowRegistrationBuildResult>> ResolveSupportedWindowRegistrations(
        WindowRegistration[] registrations)
    {
        if (registrations.Length == 0)
        {
            return BuildResult<IReadOnlyList<WindowRegistrationBuildResult>>.Unsupported("Execution IR window lowering requires at least one window registration.");
        }
        ...
        foreach (var registration in registrations)
        {
            ...
            {
                return BuildResult<IReadOnlyList<WindowRegistrationBuildResult>>.Unsupported(
        ...
        return BuildResult<IReadOnlyList<WindowRegistrationBuildResult>>.Success(results);
    }
