...
internal sealed partial class PhysicalLoweringImplementation
{
        ...
        out ExecutionVariable keyVariable)
    {
        ...
        if (hashAdd.KeyType.ResolveClrType() == typeof(object) ||
        ...
        {
            return TryCreatePrePayloadValueTupleKeyNodes(valueTupleKey, hashAdd.KeyType.ResolveClrType(), nodes, out keyVariable);
        }

        keyVariable = new ExecutionVariable("key", hashAdd.KeyType);
        nodes.Add(new ExecutionLet(keyVariable, producerKey));

        if (CanBeNullType(hashAdd.KeyType.ResolveClrType()))
        ...
        out ExecutionVariable keyVariable)
    {
        ...
        for (var index = 0; index < valueTupleKey.Parts.Count; index++)
        {
            ...
            if (CanBeNullType(part.ReturnType.ResolveClrType()))
