...
internal sealed partial class PhysicalLoweringImplementation
{
    ...
    private SidecarBuildState CreateCteSidecarIndexBuild(
        string tableName,
        GeneratedRowShape rowShape,
        CteSidecarIndexSpec spec,
        ...
        CteSidecarHashPayloadState payloads)
    {
        ...
        var nextPayloads = payloadShape == null
            ? payloads
            : payloads.WithPayload(spec.IndexSlot, payloadShape);
        return new SidecarBuildState(
            nextPayloads,
            [new CteSidecarIndexBuild(
            spec,
        ...
        out JoinSource payloadSource)
    {
        ...
        if (sidecar.Kind != CteSidecarIndexKind.Hash ||
            ...
            !scope.CteSidecarHashPayloads.TryGet(sidecar.IndexSlot, out var payloadShape))
        {
            return false;
        }
        ...
        payloadSource = source with
        {
            ...
            {
                Type = ExecutionClrBindingFactory.FromClr(typeof(Row)),
        ...
        HashPayloadShape payloadShape)
    {
        var fields = payloadShape.Fields.Select(field => field with
        {
            ...
            AccessStrategy = new GeneratedFieldAccess(ExecutionSymbolicNamePolicy.GetGeneratedFieldName(field))
        }).ToArray();
