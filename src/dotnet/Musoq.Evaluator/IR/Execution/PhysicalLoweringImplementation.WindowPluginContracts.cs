...
internal sealed partial class PhysicalLoweringImplementation
{
        ...
        PluginWindowArgumentsBuildResult arguments)
    {
        ...
        if (!CanPassValueToTypedPluginInput(arguments.Value.ReturnType.ResolveClrType(), inputType))
        {
            return CreateTypedPluginWindowDiagnostic(
                registration,
                $"the typed input {FormatType(inputType)} does not match the value argument type {FormatType(arguments.Value.ReturnType.ResolveClrType())}.");
        }
        ...
        var objectArgument = arguments.Arguments.FirstOrDefault(static argument => argument.ReturnType.ResolveClrType() == typeof(object));
        ...
        {
            var argumentTypes = arguments.Arguments.Select(static argument => argument.ReturnType.ResolveClrType()).ToArray();
