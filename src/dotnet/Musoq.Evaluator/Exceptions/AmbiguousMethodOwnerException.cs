...
public class AmbiguousMethodOwnerException : Exception, IDiagnosticException
{
    ...
    {
        Code = DiagnosticCode.MQ3035_AmbiguousMethodOwner;
    }

    public AmbiguousMethodOwnerException(string methodCall, IReadOnlyCollection<string> candidateAliases, TextSpan span)
        : base(CreateMessage(methodCall, candidateAliases))
    {
        Code = DiagnosticCode.MQ3035_AmbiguousMethodOwner;
        Span = span;
    ...
    public static string CreateMessage(string methodCall, IReadOnlyCollection<string> candidateAliases)
    {
        ...
        return $"Method call '{methodCall}' is ambiguous because multiple source aliases expose different implementations: {aliases}. Prefix the method with the intended source alias to choose the schema library implementation explicitly.";
    }
}
