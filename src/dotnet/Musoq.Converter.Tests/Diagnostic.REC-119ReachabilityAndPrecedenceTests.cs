...
public sealed class DiagnosticREC119ReachabilityAndPrecedenceTests
{
    ...
    [
        ...
        SystemCase("V06", "LET_REACHABILITY", "param (name: string); select d.Dummy from #system.dual() d", []),
        ...
        SystemCase("U08", "UNREACHABLE_BRANCH", "param (value: string); select $value ?? 'fallback' from #system.dual() d", []),
        ...
        SystemCase("U11", "UNREACHABLE_BRANCH", "param (value: bool); select case when $value then 'yes' else 'no' end from #system.dual() d", []),
