...
internal static class LoopInvariantQualificationGateCommand
{
    ...
    private static IReadOnlyList<string> ParseReports(IReadOnlyList<string> args)
    {
        ...
        if (reports.Count != LoopInvariantQualificationGate.RequiredReports)
        {
            throw new ArgumentException(
                $"Exactly {LoopInvariantQualificationGate.RequiredReports} '--report' arguments are required.");
        }
    ...
    private const string Usage =
        "Usage: gate-loop-invariant --report <benchmarkdotnet-json> (x3)";
}
