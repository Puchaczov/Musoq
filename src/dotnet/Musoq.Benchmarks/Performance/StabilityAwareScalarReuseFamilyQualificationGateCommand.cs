...
internal static class StabilityAwareScalarReuseFamilyQualificationGateCommand
{
    public static int Run(string[] args, TextWriter output, TextWriter error)
    {
        try
        {
            ...
            foreach (var comparison in result.Comparisons)
                output.WriteLine($"{comparison.Family}/{comparison.Workload}/fanout={comparison.Fanout}: " +
        ...
        catch (Exception exception) when (exception is ArgumentException or IOException or JsonException)
        {
            ...
            error.WriteLine("Usage: gate-stability-aware-reuse-families --report <benchmarkdotnet-json> (x3)");
            return 2;
        }
    }
    ...
    private static IReadOnlyList<string> ParseReports(IReadOnlyList<string> args)
    {
        ...
        if (reports.Count != StabilityAwareScalarReuseFamilyQualificationGate.RequiredReports)
            throw new ArgumentException($"Exactly {StabilityAwareScalarReuseFamilyQualificationGate.RequiredReports} '--report' arguments are required.");
        return reports;
    }
}
