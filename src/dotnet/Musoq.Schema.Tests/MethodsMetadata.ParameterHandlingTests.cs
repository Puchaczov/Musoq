...
[TestClass]
public class MethodsMetadataParameterHandlingTests : MethodsMetadataTestBase
{
    private MethodsMetadata _nullable = CreateMethodsMetadataFor<NullableMethods>();
    private MethodsMetadata _optional = CreateMethodsMetadataFor<OptionalMethods>();
    ...
    {
        Assert.IsTrue(
            _nullable.TryGetMethod("NullableInt", [typeof(NullNode.NullType)], null, out _),
    ...
    {
        Assert.IsTrue(
            _optional.TryGetMethod("SingleOptional", [typeof(int)], null, out _),
            "Should resolve with parameter"
        );

        Assert.IsTrue(
            _optional.TryGetMethod("SingleOptional", [], null, out _),
    ...
    private sealed class NullableMethods
    {
    ...
    [SuppressMessage("ReSharper", "UnusedParameter.Local")]
    private sealed class OptionalMethods
    {
