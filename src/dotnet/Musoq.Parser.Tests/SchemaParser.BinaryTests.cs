...
/// </summary>
[TestClass]
public class SchemaParserBinaryTests : SchemaParserTestsBase
{
    ...
    public void BinarySchema_NestedSchemaReference_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<SchemaReferenceTypeNode>(result);
    ...
    public void BinarySchema_SinglePrimitiveField_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<PrimitiveTypeNode>(field.TypeAnnotation);
    ...
    public void BinarySchema_ByteType_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<PrimitiveTypeNode>(result);
    ...
    public void BinarySchema_ByteArrayFixed_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<ByteArrayTypeNode>(field.TypeAnnotation);
    }
    ...
    public void BinarySchema_StringArrayWithEncoding_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<StringTypeNode>(field.TypeAnnotation);
    ...
    public void BinarySchema_BitsType_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<BitsTypeNode>(field.TypeAnnotation);
    }
    ...
    public void BinarySchema_AlignType_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<AlignmentNode>(field.TypeAnnotation);
    }
    ...
    public void BinarySchema_PrimitiveArrayWithEndianness_ShouldParse()
    {
        ...
        var field = result.Fields[0] as FieldDefinitionNode;
        Assert.IsNotNull(field);
        Assert.IsInstanceOfType<ArrayTypeNode>(field.TypeAnnotation);

        var arrayType = (ArrayTypeNode)field.TypeAnnotation;
        Assert.IsInstanceOfType<PrimitiveTypeNode>(arrayType.ElementType);
    ...
    public void BinarySchema_SchemaReferenceArray_ShouldParse()
    {
        ...
        var arrayType = (ArrayTypeNode)field.TypeAnnotation;
        Assert.IsInstanceOfType<SchemaReferenceTypeNode>(arrayType.ElementType);
    ...
    public void BinarySchema_ByteArrayWithArithmeticSize_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<ByteArrayTypeNode>(dataField.TypeAnnotation);

        var byteArrayType = (ByteArrayTypeNode)dataField.TypeAnnotation;

        Assert.IsInstanceOfType<HyphenNode>(byteArrayType.SizeExpression);
        var hyphenNode = (HyphenNode)byteArrayType.SizeExpression;

        Assert.IsInstanceOfType<IdentifierNode>(hyphenNode.Left);
        Assert.IsInstanceOfType<IdentifierNode>(hyphenNode.Right);
    ...
    public void BinarySchema_RepeatUntil_SimpleCondition_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<RepeatUntilTypeNode>(field.TypeAnnotation);

        var repeatType = (RepeatUntilTypeNode)field.TypeAnnotation;
        Assert.IsInstanceOfType<SchemaReferenceTypeNode>(repeatType.ElementType);
    ...
    public void BinarySchema_RepeatUntil_WithPrimitiveType_ShouldParse()
    {
        ...
        var repeatType = (RepeatUntilTypeNode)field.TypeAnnotation;
        Assert.IsInstanceOfType<PrimitiveTypeNode>(repeatType.ElementType);
    }

    [TestMethod]
    public void BinarySchema_RepeatUntil_WithBitsType_ShouldParse()
    {
        var schema = "binary Stream { Flags: bits[1] repeat until Flags = 0 }";
        var result = ParseBinarySchema(schema);
        ...
        var repeatType = (RepeatUntilTypeNode)field.TypeAnnotation;
        Assert.IsInstanceOfType<BitsTypeNode>(repeatType.ElementType);
        Assert.AreEqual("Flags", repeatType.FieldName);
        Assert.IsInstanceOfType<EqualityNode>(repeatType.Condition);
    }

    [TestMethod]
    public void BinarySchema_RepeatUntil_WithStringType_ShouldParse()
    {
        var schema = "binary Stream { Names: string[3] ascii repeat until Names = 'END' }";
        var result = ParseBinarySchema(schema);
        ...
        var repeatType = (RepeatUntilTypeNode)field.TypeAnnotation;
        Assert.IsInstanceOfType<StringTypeNode>(repeatType.ElementType);
        Assert.AreEqual("Names", repeatType.FieldName);
        Assert.IsInstanceOfType<EqualityNode>(repeatType.Condition);
    }

    [TestMethod]
    public void BinarySchema_RepeatUntil_WithInlineSchemaType_ShouldParse()
    {
        var schema = "binary Stream { Items: { Tag: byte, Value: short le } repeat until Items.Tag = 0 }";
        var result = ParseBinarySchema(schema);
        ...
        var repeatType = (RepeatUntilTypeNode)field.TypeAnnotation;
        Assert.IsInstanceOfType<InlineSchemaTypeNode>(repeatType.ElementType);
        Assert.AreEqual("Items", repeatType.FieldName);
        Assert.IsInstanceOfType<EqualityNode>(repeatType.Condition);
    }
    ...
    [TestMethod]
    public void BinarySchema_RepeatUntilEof_WithPrimitiveType_ShouldParse()
    {
        var schema = "binary Stream { Bytes: byte repeat until eof }";
        var result = ParseBinarySchema(schema);
        ...
        var repeatType = (RepeatUntilTypeNode)field.TypeAnnotation;
        Assert.AreEqual(RepeatUntilStopKind.EndOfInput, repeatType.StopKind);
        Assert.IsNull(repeatType.Condition);
        Assert.IsInstanceOfType<PrimitiveTypeNode>(repeatType.ElementType);
    }

    [TestMethod]
    public void BinarySchema_RepeatUntilEof_CaseInsensitive_ShouldParse()
    {
        var schema = "binary Stream { Bytes: byte repeat until EOF }";
        var result = ParseBinarySchema(schema);
    ...
    [TestMethod]
    public void BinarySchema_RepeatUntilEof_ToString_ShouldRenderEof()
    {
        var schema = "binary Stream { Bytes: byte repeat until eof }";
        var result = ParseBinarySchema(schema);
        ...
        Assert.EndsWith("repeat until eof", repeatType.ToString());
    }

    [TestMethod]
    public void BinarySchema_RepeatUntilCondition_StopKind_ShouldBeCondition()
    {
        var schema = "binary Stream { Records: Record repeat until Values = 0 }";
        var result = ParseBinarySchema(schema);

        var field = result.Fields[0] as FieldDefinitionNode;
        var repeatType = (RepeatUntilTypeNode)field!.TypeAnnotation;

        Assert.AreEqual(RepeatUntilStopKind.Condition, repeatType.StopKind);
        Assert.IsNotNull(repeatType.Condition);
    }
    ...
    {
        var lexer = new Lexer("byte[16]", true) { IsSchemaContext = true };
