...
/// </summary>
[TestClass]
public class SchemaParserAdvancedTests : SchemaParserTestsBase
{
    #region Error Cases
    ...
    [TestMethod]
    public void BinarySchema_BitsCountZero_ShouldThrowSyntaxException()
    {
        var schema = "binary Name { Flags: bits[0] }";

        Assert.Throws<SyntaxException>(() => ParseBinarySchema(schema));
    }

    [TestMethod]
    public void BinarySchema_BitsCountAboveSixtyFour_ShouldThrowSyntaxException()
    {
        var schema = "binary Name { Flags: bits[65] }";

        Assert.Throws<SyntaxException>(() => ParseBinarySchema(schema));
    }

    [TestMethod]
    public void BinarySchema_AlignmentZero_ShouldThrowSyntaxException()
    {
        var schema = "binary Name { _: align[0] }";

        Assert.Throws<SyntaxException>(() => ParseBinarySchema(schema));
    }

    [TestMethod]
    public void BinarySchema_NegativePrimitiveArraySize_ShouldThrowSyntaxException()
    {
        var schema = "binary Name { Values: int[-1] le }";
    ...
    [TestMethod]
    public void BinarySchema_NestedGenericTypeInstantiation_ShouldParse()
    {
        var schema = "binary Container { Items: LengthPrefixed<Pair<Header,Footer>> }";
        ...
        var schemaRef = itemsField.TypeAnnotation as SchemaReferenceTypeNode;
        Assert.IsNotNull(schemaRef);
        Assert.AreEqual("LengthPrefixed", schemaRef.SchemaName);
        Assert.HasCount(1, schemaRef.TypeArguments);
        Assert.AreEqual("Pair<Header, Footer>", schemaRef.TypeArguments[0]);
        Assert.AreEqual("LengthPrefixed<Pair<Header, Footer>>", schemaRef.FullTypeName);
    }
    ...
    public void BinarySchema_InlineSchema_SimpleFields_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<InlineSchemaTypeNode>(headerField.TypeAnnotation);
        ...
        Assert.IsInstanceOfType<PrimitiveTypeNode>(magicField.TypeAnnotation);
    ...
    [TestMethod]
    public void BinarySchema_InlineSchemaArray_ShouldParse()
    {
        var schema = @"binary Packet {
            Count: byte,
        ...
        Assert.HasCount(2, result.Fields);
        var itemsField = result.Fields[1] as FieldDefinitionNode;
        Assert.IsNotNull(itemsField);
        Assert.AreEqual("Items", itemsField.Name);
        Assert.IsInstanceOfType<ArrayTypeNode>(itemsField.TypeAnnotation);

        var arrayType = (ArrayTypeNode)itemsField.TypeAnnotation;
        Assert.IsInstanceOfType<InlineSchemaTypeNode>(arrayType.ElementType);

        var inlineSchema = (InlineSchemaTypeNode)arrayType.ElementType;
        Assert.HasCount(2, inlineSchema.Fields);
        Assert.AreEqual("Tag", inlineSchema.Fields[0].Name);
        Assert.AreEqual("Value", inlineSchema.Fields[1].Name);
        Assert.IsInstanceOfType<IdentifierNode>(arrayType.SizeExpression);
    }
    ...
    public void BinarySchema_InlineSchema_EmptyFields_ShouldParse()
    {
        ...
        var dataField = result.Fields[0] as FieldDefinitionNode;
        Assert.IsNotNull(dataField);
        Assert.IsInstanceOfType<InlineSchemaTypeNode>(dataField.TypeAnnotation);
    ...
    public void BinarySchema_InlineSchema_NestedInline_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<InlineSchemaTypeNode>(outerField.TypeAnnotation);
        ...
        Assert.IsInstanceOfType<InlineSchemaTypeNode>(innerField.TypeAnnotation);
    ...
    public void BinarySchema_InlineSchema_MixedWithRegularFields_ShouldParse()
    {
        ...
        Assert.IsInstanceOfType<PrimitiveTypeNode>(preambleField.TypeAnnotation);
