...
[TestClass]
public class HashOptionalSchemaParserEdgeCaseTests
{
    [TestMethod]
    ...
    [TestMethod]
    public void HashOptional_PostfixCastSyntax_ShouldParse()
    {
        var query = "select Col1::Int32, Count(Col1::Int32), Col2::String from schema.method() group by Col1, Col2";
        var lexer = new Lexer(query, true);
        var parser = new Parser(lexer);
        var result = parser.ComposeAll();
        Assert.IsNotNull(result);
