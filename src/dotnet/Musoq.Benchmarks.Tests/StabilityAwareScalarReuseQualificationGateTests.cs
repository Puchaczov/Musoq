...
[TestClass]
public sealed class StabilityAwareScalarReuseQualificationGateTests
{
    ...
    public void Initialize()
    {
        _directory = Path.Combine(
            Path.GetTempPath(),
            "musoq-stability-aware-reuse-gate",
    ...
    public void Evaluate_WhenThreeCompleteCohortsMeetThresholds_ShouldPass()
    {
        ...
        Assert.HasCount(12, result.Comparisons);
        Assert.IsEmpty(result.Failures);
        Assert.AreEqual(
            0.95d,
            result.Comparisons.Single(comparison =>
                comparison.Scenario == "StableExpensiveFilter" && comparison.Fanout == 64).TimeRatio,
            0.0001d);
    }
    ...
    public void Evaluate_WhenExpensiveHighFanoutMissesRatio_ShouldFail()
    {
        ...
        StringAssert.Contains(
            ...
            "StableExpensiveFilter/fanout=64");
    }
    ...
    public void Evaluate_WhenAReportIsIncomplete_ShouldRejectIt()
    {
        ...
        File.WriteAllText(incomplete, JsonSerializer.Serialize(new
        {
            Benchmarks = new[]
            {
                new
                {
                    FullName = "ExecuteOff(Fanout: 1, Scenario: StableCheapFilter)",
    ...
    public void Command_WhenThreeReportsAreProvided_ShouldEmitMachineReadableResult()
    {
        ...
        var exitCode = StabilityAwareScalarReuseQualificationGateCommand.Run(
    ...
    {
        return Enumerable.Range(1, StabilityAwareScalarReuseQualificationGate.RequiredReports)
    ...
    private string WriteReport(string name, double onTime, double offTime, double onAllocation, double offAllocation)
    {
        ...
        foreach (var scenario in new[]
                 {
                     "StableCheapFilter", "StableExpensiveFilter", "StableAggregate", "VolatileFilter"
                 })
