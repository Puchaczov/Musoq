...
public class SingleSchemaQueriesAndAggregationTests : BasicEntityTestBase
{
    ...
    [TestMethod]
    public void AggregateValues_WithCommaDelimiter_ShouldConcatenateGroupedValues()
    {
        var query = @"select AggregateValues(Name, ', ') from #A.entities() a group by 'const'";
        ...
        Assert.AreEqual(1, table.Count);
        Assert.AreEqual("Alice, Bob, Carol", table[0][0]);
    }

    [TestMethod]
    public void AggregateValues_WithPipeDelimiter_ShouldConcatenateGroupedValues()
    {
        var query = @"select AggregateValues(Name, ' | ') from #A.entities() a group by 'const'";
        ...
        Assert.AreEqual(1, table.Count);
        Assert.AreEqual("X | Y | Z", table[0][0]);
    }

    [TestMethod]
    public void AggregateValues_WithGroupBy_ShouldConcatenatePerGroup()
    {
        var query = @"select City, AggregateValues(Name, ', ') from #A.entities() a group by City";
        ...
        var nycRow = table.First(r => (string)r[0] == "NYC");
        var laRow = table.First(r => (string)r[0] == "LA");

        Assert.AreEqual("Alice, Carol", nycRow[1]);
        Assert.AreEqual("Bob", laRow[1]);
    }

    [TestMethod]
    public void AggregateValues_WithSingleValue_ShouldReturnJustTheValue()
    {
        var query = @"select AggregateValues(Name, ', ') from #A.entities() a group by Name";
