...
public class StringHelpersTests
{
    ...
    public void GenerateNamespaceIdentifier_WhenCalledConcurrently_ShouldReturnUniqueValues()
    {
        ...
        Parallel.For(0, iterations, _ => { results.Add(StringHelpers.GenerateNamespaceIdentifier()); });
    ...
    public void GenerateNamespaceIdentifier_WhenCalledFromMultipleThreads_ShouldBeThreadSafe()
    {
        ...
        var tasks = Enumerable.Range(0, threadCount)
            .Select(_ => Task.Run(() =>
            {
                for (var i = 0; i < iterationsPerThread; i++) results.Add(StringHelpers.GenerateNamespaceIdentifier());
            }))
            .ToArray();
    ...
    public void GenerateNamespaceIdentifier_HighContention_ShouldNotLoseIncrements()
    {
        ...
        Parallel.For(0, iterations, options, _ => { results.Add(StringHelpers.GenerateNamespaceIdentifier()); });
