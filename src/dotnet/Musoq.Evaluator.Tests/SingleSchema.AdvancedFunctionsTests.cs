...
public class SingleSchemaAdvancedFunctionsTests : BasicEntityTestBase
{
    ...
    [TestMethod]
    public void QueryWithNullableDateTimeSubtraction_ShouldReturnDifference()
    {
        var query = "select ToDateTime('2012/01/14') - ToDateTime('2012/01/13') from #A.entities()";
        ...
        Assert.AreEqual(1, table.Count);
        Assert.AreEqual(TimeSpan.FromDays(1), table[0][0]);
    }

    [TestMethod]
    public void QueryWithNullableDateTimeSubtraction_WhenConversionFails_ShouldReturnNull()
    {
        var query = "select ToDateTime('not-a-date') - ToDateTime('2012/01/13') from #A.entities()";
