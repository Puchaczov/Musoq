...
public class SemanticLogicalTests : NegativeTestsBase
{
    ...
    {
        var vm = CompileQuery(
            "SELECT p.Name, e.Name FROM #test.people() p LEFT OUTER JOIN #test.empty() e ON p.Id = e.Id");
        var table = vm.Run(CancellationToken.None);
        Assert.AreEqual(5, table.Count);
    }
    ...
    {
        var vm = CompileQuery(
            "SELECT a.Name, b.Name FROM #test.people() a INNER JOIN #test.people() b ON a.City = b.City WHERE a.Name <> b.Name");
        var table = vm.Run(CancellationToken.None);
