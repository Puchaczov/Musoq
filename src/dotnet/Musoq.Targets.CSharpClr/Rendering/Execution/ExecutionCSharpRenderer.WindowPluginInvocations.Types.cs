...
public sealed partial class ExecutionCSharpRenderer
{
        ...
        out Type resultType)
    {
        ...
        if (!plugin.Results.Type.RequireClrType().IsArray)
            return false;

        resultType = plugin.Results.Type.RequireClrType().GetElementType() ?? typeof(object);
        if (resultType == typeof(object))
            return false;

        if (!TryGetPluginWindowTypes(plugin.FactoryMethod.RequireClrMethod(), out inputType, out var factoryResultType))
            return false;

        if (inputType == typeof(object) || factoryResultType == typeof(object))
            return false;

        return CanPassValueToTypedPluginInput(plugin.Value.ReturnType.RequireClrType(), inputType) &&
