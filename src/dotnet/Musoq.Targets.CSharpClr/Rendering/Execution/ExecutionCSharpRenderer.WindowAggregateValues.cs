...
public sealed partial class ExecutionCSharpRenderer
{
    ...
    {
        IReadOnlyList<StatementSyntax> statements = Nullable.GetUnderlyingType(kernel.Descriptor.InputType.RequireClrType()) != null
        ...
        ExecutionWindowAggregateKernel kernel)
    {
        switch (kernel.Descriptor.Function)
        {
            ...
            case ExecutionWindowAggregateFunction.Count:
                if (!kernel.Descriptor.InputType.RequireClrType().IsValueType)
                {
        ...
        ExecutionWindowAggregateKernel kernel)
    {
        ...
        yield return CreateWindowAggregateMinMaxUpdateStatement(
            kernel,
            CreateWindowAggregateMinMaxValueReadExpression(valueName, kernel.Descriptor.InputType.RequireClrType()));
    }
    ...
    {
        return Nullable.GetUnderlyingType(kernel.Descriptor.InputType.RequireClrType()) ?? kernel.Descriptor.InputType.RequireClrType();
    }
}
