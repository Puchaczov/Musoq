...
public sealed partial class ExecutionCSharpRenderer
{
        ...
        ExecutionRenderContext context)
    {
        if (methodCall.Target == null)
            throw new NotSupportedException($"Cached method {methodCall.Method.MethodName} requires a reusable target.");
        if (methodCall.Arguments.Count != 1)
            throw new NotSupportedException($"Cached method {methodCall.Method.MethodName} requires exactly one argument.");
