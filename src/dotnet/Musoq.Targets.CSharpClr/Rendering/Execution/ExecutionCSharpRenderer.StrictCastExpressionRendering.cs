...
public sealed partial class ExecutionCSharpRenderer
{
    private ExpressionSyntax RenderStrictCast(ExecutionStrictCast strictCast, ExecutionRenderContext context)
    {
        ...
        var sourceType = strictCast.Expression.ReturnType.RequireClrType();
        var targetType = strictCast.ReturnType.RequireClrType();

        if (StrictCastLibraryConversionFacts.IsPassThrough(sourceType, targetType))
            return CastExpressionIfNeeded(source, targetType, sourceType);

        if (sourceType == typeof(DBNull))
            return CreateTypedNull(targetType);

        var methodName = GetLibraryConversionMethodName(StrictCastLibraryConversionFacts.GetCastTargetType(targetType));
        ...
        return CastExpressionIfNeeded(invocation, targetType, targetType);
    }
