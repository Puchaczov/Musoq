...
public partial class LibraryBase
{
    ...
    public IEnumerable<DiffSegmentEntity> DiffSegments(string? first, string? second)
    {
        ...
        if (first == null)
        {
            var inserted = second ?? string.Empty;
            return [new DiffSegmentEntity(inserted, KindInserted, 0, inserted.Length)];
        }
    ...
    private static string BuildDiffString(List<DiffSegmentEntity> segments, string modeName, int? threshold)
    {
        ...
        foreach (var segment in segments)
            switch (segment.Kind)
            {
                case KindUnchanged:
                    if (modeName == "compact")
                        result.Append(System.Globalization.CultureInfo.InvariantCulture, $"[={segment.Length}]");
                ...
                case KindDeleted:
                    result.Append(System.Globalization.CultureInfo.InvariantCulture, $"[-{segment.Text}]");
                    break;
