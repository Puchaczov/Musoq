...
public static class AggregateValuesCharKernel
{
    public struct State
    {
    ...
    public static void Set(ref State state, char? value)
        => AggregateCollectionCore.AddOrdered(ref state.Values, value?.ToString() ?? string.Empty);
