...
public class CrossApplyBugTestCases : GenericEntityTestBase
{
    ...
    {
        const string query =
            "select a.Key, a.Value, b.FilterKey, b.Amount from #schema.first() a cross apply #schema.second(a.Key) b";
    ...
    {
        const string query =
            "select a.Key, a.Value, b.FilterKey, b.Amount from #schema.first() a cross apply #schema.second(a.Key, a.Value) b";
