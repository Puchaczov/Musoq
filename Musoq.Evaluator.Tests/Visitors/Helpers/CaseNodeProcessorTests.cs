...
public class CaseNodeProcessorTests
{
    ...
    public void ProcessCaseNode_WithTransformingQuery_UsesCorrectRowVariableName()
    {
        ...
        // Act
        var result = CaseNodeProcessor.ProcessCaseNode(
            caseNode, nodes, typesToInstantiate, MethodAccessType.TransformingQuery, "myQuery",
            ref caseWhenMethodIndex);
    ...
    public void ProcessCaseNode_WithComplexNestedStructure_GeneratesCorrectSyntax()
    {
        ...
        var nodes = new Stack<SyntaxNode>();
        nodes.Push(
            SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal("default")));
        nodes.Push(
            SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal("result3")));
        ...
        nodes.Push(
            SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal("result2")));
        nodes.Push(SyntaxFactory.BinaryExpression(SyntaxKind.EqualsExpression,
            SyntaxFactory.IdentifierName("x"),
            SyntaxFactory.LiteralExpression(SyntaxKind.NumericLiteralExpression, SyntaxFactory.Literal(2))));
        nodes.Push(
            SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal("result1")));
        nodes.Push(SyntaxFactory.BinaryExpression(SyntaxKind.EqualsExpression,
            SyntaxFactory.IdentifierName("x"),
