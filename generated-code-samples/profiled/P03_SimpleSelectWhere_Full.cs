...
namespace GeneratedSample_P03_SimpleSelectWhere_Full
{
    ...
    public sealed class CompiledQuery : BaseOperations, ITableRunnable, IParameterizedRunnable, IProfiledRunnable
    {
        ...
        {
            foreach (var __musoqShapeRow in ProfiledOperatorEnumerable<ResultShape0>.Create(ComputeShapeRows_compiled_0(provider, sourceRuntimeSettingsBySourceContextId, sourceExecutionPlans, logger, token, profileRecorder), profileRecorder, profileRecorder?.GetCurrentOperatorScopeDepth() ?? 0))
            {
