...
ExecutionPlan [compiled]
  ...
  Body
    ...
    SortTable [cte0 -> cte0Sorted by p.Value ASC]
    StoreTable [cte0Sorted -> _tableResults[0]]
...
// === SyntaxTree:  ===
namespace GeneratedSample_Q341_StructuredSortedCteArgument
{
    using System;
        ...
        private IEnumerable<ResultShape0> ComputeShapeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    var cte0SortedRows = EvaluationHelper.CastGeneratedRows<Cte0Row0>(cte0.Rows).OrderBy((row) => row, Cte0Row0OrderBy_0AComparer.Instance);
                    var cte0Sorted = new Table("cte0Sorted", __columns_compiled_cte0_0);
                    cte0Sorted.EnsureCapacity(cte0.Count);
                    foreach (var copiedRow in cte0SortedRows)
                    {
                        cte0Sorted.AddDirect(copiedRow);
                    }

                    _tableResults[0] = cte0Sorted;
                }
                finally
                {
