...
// === SyntaxTree:  ===
namespace GeneratedSample_Q293_SpecCoreDescSchema
{
    using System;
    ...
    public sealed class CompiledQuery : BaseOperations, ITableRunnable, IQueryProgressSource, IParameterizedRunnable
    {
        ...
        private Table ComputeTable_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            var descRuntimeCtx = new SourceExecutionContext("df8apb:7", SourceExecutionPlan.Empty(new SourceIdentity("#A", "", "df8apb:7", "")), token, emptyInferred, sourceRuntimeSettingsBySourceContextId.TryGetValue("df8apb:7", out var descSourceRuntimeSettings) ? descSourceRuntimeSettings : new Dictionary<string, string>(), logger, OnDataSourceProgress);
            try
            {
            ...
            return EvaluationHelper.GetSpecificSchemaDescriptions(descSchema, descRuntimeCtx);
