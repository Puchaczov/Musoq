// === Parsed Query ===
/*
with recursive reachable (Id, Depth) as (select RootId, 0 from #graph.roots() union (Id) select e.TargetId, r.Depth + 1 from reachable r inner join #graph.edges() e on e.SourceId = r.Id), deep (Id, Depth) as (select Id, Depth from reachable where Depth > 0) select Id, Depth from deep
*/
  ...
  Definition [deep]
    MultiStatement
      Project [reachable.Id as Id, reachable.Depth as Depth]
        Filter [(reachable.Depth > 0)]
          CteRef [reachable as reachable]
  Query
    MultiStatement
      Project [deep.Id as Id, deep.Depth as Depth]
        CteRef [deep as deep]
*/
  ...
  Definition [deep]
    PhysicalMultiStatement
      PhysicalProject [reachable.Id as Id, reachable.Depth as Depth]
        PhysicalFilter [(reachable.Depth > 0)]
          PhysicalCteRef [reachable as reachable]
  Query
    PhysicalMultiStatement
      PhysicalProject [deep.Id as Id, deep.Depth as Depth]
        PhysicalCteRef [deep as deep]
*/
...
ExecutionPlan [compiled]
  ...
  Body
    ...
    ForEach [reachable in _cteRowResults.Slot0]
      Let [depth: int = reachable.Depth]
      If [(depth > 0)]
        AppendShape [result <- ResultShape0(Id: reachable.Id, Depth: depth)]
...
// === SyntaxTree:  ===
namespace GeneratedSample_Q206_RecursiveDependentOrdinaryCte
{
    using System;
            ...
            try
            {
                ...
                try
                {
                    ...
                    for (int __storedTable0Index = 0; __storedTable0Index < __storedTable0Rows.Count; ++__storedTable0Index)
                    {
                        ...
                        int depth = reachable.Depth;
                        if ((depth > 0))
                        {
                            __musoqFinalShapeRows.Add(new ResultShape0(reachable.Id, depth));
                        }
                    }
