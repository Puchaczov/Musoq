...
MultiStatement
  Project [ko3iko.Name as Name, ko3iko.Department as Department, ko3iko.Salary as Salary, WindowRef(0) as SalaryRank]
    Qualify [(WindowRef(0) <= 3)]
      Window [Rank(idx:0; partition: ko3iko.Department; order: ko3iko.Salary DESC)]
...
PhysicalMultiStatement
  PhysicalProject [ko3iko.Name as Name, ko3iko.Department as Department, ko3iko.Salary as Salary, WindowRef(0) as SalaryRank]
    PhysicalQualify [(WindowRef(0) <= 3)]
      PhysicalWindow [Rank(idx:0; partition: ko3iko.Department; order: ko3iko.Salary DESC)]
        PhysicalMaterialize
...
ExecutionPlan [compiled]
  ...
  Body
    ...
    ComputeRankWindow [resultRanks <- resultWindowRows partition by ko3iko.Department order by ko3iko.Salary DESC qualify <= 3]
    CreateShapeRows [result: ResultShape0 from ResultRow0]
    PhaseBoundary [Select]
    ForEachIndexed [windowIndex, ko3iko in resultWindowRows]
      If [((resultRanks[windowIndex] > 0) AND (resultRanks[windowIndex] <= 3))]
...
namespace GeneratedSample_Q102_RuntimeV2WindowQualifyRank
{
    ...
    public sealed class CompiledQuery : BaseOperations, ITableRunnable, IQueryProgressSource, IParameterizedRunnable
    {
        private static readonly Column[] __columns_compiled_result_1 = new Column[]
        {
            ...
            new Column("SalaryRank", typeof(long), 3)
        };
        private static readonly IReadOnlyCollection<ISchemaColumn> __schemaColumns_compiled_ko3iko_0 = Array.AsReadOnly(new ISchemaColumn[] { new Column("Name", typeof(string), 0), new Column("Department", typeof(string), 1), new Column("Salary", typeof(int), 2) });
        ...
        private IEnumerable<ResultRow0> ComputeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            {
                yield return new ResultRow0(__musoqShapeRow.Name, __musoqShapeRow.Department, __musoqShapeRow.Salary, __musoqShapeRow.SalaryRank);
            }
        }
        ...
        private IEnumerable<ResultShape0> ComputeShapeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            try
            {
                ...
                var resultRanksIntOrderBuilder = new Musoq.Evaluator.Helpers.WindowIntOrderBuilder<string>(resultWindowRows.Count);
                for (int windowIndex = 0; windowIndex < resultWindowRows.Count; ++windowIndex)
                {
                    ...
                    resultRanksIntOrderBuilder.Add((string)ko3iko.Department, (int)ko3iko.Salary, windowIndex);
                }

                var resultRanksPartitions = resultRanksIntOrderBuilder.ToSortedPartitionSet(true);
                var resultRanks = resultRanksIntOrderBuilder.ComputeRankTopN(resultRanksPartitions, 3L);
                ...
                for (int windowIndex = 0; windowIndex < resultWindowRows.Count; ++windowIndex)
                {
                    ...
                    Musoq.Evaluator.Tests.Schema.RuntimeV2.RuntimeV2RegressionEntity ko3iko = resultWindowRows[windowIndex];
                    if ((((long)resultRanks[windowIndex] > 0L) && ((long)resultRanks[windowIndex] <= 3)))
                    {
                        __musoqFinalShapeRows.Add(new ResultShape0(ko3iko.Name, ko3iko.Department, ko3iko.Salary, (long)resultRanks[windowIndex]));
                    }
                }
        ...
        {
            public ResultRow0(string __value0, string __value1, int __value2, long __value3)
            {
                Name = __value0;
                Department = __value1;
                Salary = __value2;
        ...
        {
            public ResultShape0(string Name, string Department, int Salary, long SalaryRank)
            {
                this.Name = Name;
