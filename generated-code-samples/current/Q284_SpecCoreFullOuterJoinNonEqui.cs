...
PhysicalMultiStatement
  PhysicalProject [a.Name as a.Name, a.Id as a.Id, b.Name as b.Name, b.Id as b.Id]
    PhysicalNestedLoopJoin [FullOuter] [(a.Id < b.Id)]
...
ExecutionPlan [compiled]
  ...
  Body
    ...
    MaterializeChunked [aRows -> aRowsBuffer]
    MaterializeChunked [bRows -> bRowsBuffer]
    ForEach [a in aRowsBuffer]
      Let [resultLeftHasMatch: bool = FALSE]
      ForEach [b in bRowsBuffer]
        If [(a.Id < b.Id)]
          Assign [resultLeftHasMatch = TRUE]
          AppendShape [result <- ResultShape0(a.Name: a.Name, b.Name: b.Name)]
      If [NOT resultLeftHasMatch]
        AppendShape [result <- ResultShape0(a.Name: a.Name, b.Name: NULL)]
    ForEach [b in bRowsBuffer]
      Let [resultRightHasMatch: bool = FALSE]
      ForEach [a in aRowsBuffer]
        If [(a.Id < b.Id)]
          Assign [resultRightHasMatch = TRUE]
      If [NOT resultRightHasMatch]
        AppendShape [result <- ResultShape0(a.Name: NULL, b.Name: b.Name)]
...
// === SyntaxTree:  ===
namespace GeneratedSample_Q284_SpecCoreFullOuterJoinNonEqui
{
    using System;
        ...
        private IEnumerable<ResultShape0> ComputeShapeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    var aRowsBuffer = EvaluationHelper.MaterializeChunkedRows(aRows);
                    var bRowsBuffer = EvaluationHelper.MaterializeChunkedRows(bRows);
                    foreach (var a in aRowsBuffer)
                    {
                        token.ThrowIfCancellationRequested();
                        bool resultLeftHasMatch = false;
                        foreach (var b in bRowsBuffer)
                        {
                            token.ThrowIfCancellationRequested();
                            if ((Operators.SqlCompare<int?, int?>(a.Id, b.Id, (int? __sqlLeft, int? __sqlRight) => (__sqlLeft < __sqlRight))) == true)
                            {
                                resultLeftHasMatch = true;
                                __musoqFinalShapeRows.Add(new ResultShape0(a.Name, b.Name));
                            }
                        }

                        if ((!resultLeftHasMatch))
                        {
                    ...
                    {
                        token.ThrowIfCancellationRequested();
                        bool resultRightHasMatch = false;
                        foreach (var a in aRowsBuffer)
                        {
                            token.ThrowIfCancellationRequested();
                            if ((Operators.SqlCompare<int?, int?>(a.Id, b.Id, (int? __sqlLeft, int? __sqlRight) => (__sqlLeft < __sqlRight))) == true)
                            {
                                resultRightHasMatch = true;
                            }
                        }

                        if ((!resultRightHasMatch))
                        {
                            __musoqFinalShapeRows.Add(new ResultShape0(null, b.Name));
                        }
                    }
                }
                finally
