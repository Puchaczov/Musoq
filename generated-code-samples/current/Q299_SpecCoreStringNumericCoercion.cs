...
MultiStatement
  Project [ko3iko.Name as Name]
    Filter [(TryConvertToInt32Strict(ko3iko.Name) = 42)]
...
PhysicalMultiStatement
  PhysicalProject [ko3iko.Name as Name]
    PhysicalFilter [(TryConvertToInt32Strict(ko3iko.Name) = 42)]
...
ExecutionPlan [compiled]
  ...
  Body
    ...
    ParallelFilterProjectLoop [ko3iko in ko3ikoRows where (TryConvertToInt32Strict(ko3iko.Name) = 42); threshold 4096, maxDegree 24]
      ParallelProject
        Let [name: string = ko3iko.Name]
        If [(TryConvertToInt32Strict(name) = 42)]
...
// === SyntaxTree:  ===
namespace GeneratedSample_Q299_SpecCoreStringNumericCoercion
{
    using System;
    ...
    public sealed class CompiledQuery : BaseOperations, ITableRunnable, IQueryProgressSource, IParameterizedRunnable
    {
        ...
        private IEnumerable<ResultRow0> ComputeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            {
                return new QueryTableEnumerable<ResultRow0>((_) => EvaluationHelper.ProjectChunkedRowsParallel<Musoq.Evaluator.Tests.Schema.Basic.BasicEntity, ResultRow0>(__musoqTableSourceRows, 24, (ko3iko) => (Operators.SqlCompare<int?, int>((int?)__resultLibraryBase0.TryConvertToInt32Strict(ko3iko.Name), 42, (int? __sqlLeft, int __sqlRight) => (__sqlLeft == __sqlRight))) == true, (ko3iko) => new ResultRow0(ko3iko.Name), token), token, onCompleted: () =>
                {
                    try
                    {
