// === Parsed Query ===
/*
select Country, Count(*) filter (where Population > 200) as FilteredCount from #A.entities() group by Country
*/

// === Logical Plan ===
/*
MultiStatement
  Project [ko3iko.Country as ko3iko.Country, AggRef(ko3iko.Count(*) filter (where Population > 200)) as ko3iko.Count(*) filter (where Population > 200)]
    Aggregate [keys: Country] [aggs: Count(*) filter (where Population > 200)]
      SchemaScan [#A.entities() as ko3iko]
  Project [ko3iko.Country as Country, ko3iko.Count(*) filter (where Population > 200) as FilteredCount]
...
PhysicalMultiStatement
  PhysicalProject [ko3iko.Country as ko3iko.Country, AggRef(ko3iko.Count(*) filter (where Population > 200)) as ko3iko.Count(*) filter (where Population > 200)]
    PhysicalSingleKeyAggregate [key: Country (String)] [aggs: Count(*) filter (where Population > 200)]
      PhysicalSchemaScan [#A.entities() as ko3iko]
  PhysicalProject [ko3iko.Country as Country, ko3iko.Count(*) filter (where Population > 200) as FilteredCount]
...
ExecutionPlan [compiled]
  Shapes
    ...
    Generated [ResultRow0]
      ...
      FilteredCount: long <- field FilteredCount
  ...
  Body
    ...
    ParallelSingleKeyAggregateLoop [ko3iko in ko3ikoRows by ko3iko.Country; threshold 4096, sample 8192/6144, maxDegree 24, group ResultAggregateGroup]
      ParallelAccumulate
        TypedAggregateSet [Set(group.__agg0) filter (ko3iko.Population > 200)]
    EnsureShapeCapacity [result <- groupsToFinalize.Count]
    PhaseBoundary [Select]
    ForEach [finalGroup in groupsToFinalize]
      AppendShape [result <- ResultShape0(Country: finalGroup.Country, FilteredCount: ko3iko.Count(*) filter (where Population > 200))]
...
// === SyntaxTree:  ===
namespace GeneratedSample_Q287_SpecCoreAggregateFilter
{
    using System;
    ...
    public sealed class CompiledQuery : BaseOperations, ITableRunnable, IQueryProgressSource, IParameterizedRunnable
    {
        private static readonly Column[] __columns_compiled_result_1 = new Column[]
        {
            ...
            new Column("FilteredCount", typeof(long), 1)
        };
        private static readonly IReadOnlyCollection<ISchemaColumn> __schemaColumns_compiled_ko3iko_0 = Array.AsReadOnly(new ISchemaColumn[] { new Column("Country", typeof(string), 0), new Column("Population", typeof(decimal), 1) });
        ...
        private IEnumerable<ResultRow0> ComputeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            {
                yield return new ResultRow0(__musoqShapeRow.Country, __musoqShapeRow.FilteredCount);
            }
        }
        ...
        {
            public ResultShape0(string Country, long FilteredCount)
            {
