...
Cte
  Definition [_sq_1]
    MultiStatement
      Project [1 as _sq_1_key, b.Country as _sq_1_corr_0]
        SchemaScan [#B.entities() as b]
  Query
    MultiStatement
      Project [a.City as a.City]
        Join [LeftAntiSemi] [((1 = _sq_1._sq_1_key) AND (_sq_1._sq_1_corr_0 = a.Country))]
...
PhysicalCte
  Definition [_sq_1]
    PhysicalMultiStatement
      PhysicalProject [1 as _sq_1_key, b.Country as _sq_1_corr_0]
        PhysicalSchemaScan [#B.entities() as b]
  Query
    PhysicalMultiStatement
      PhysicalProject [a.City as a.City]
        PhysicalHashJoin [LeftAntiSemi] [build: _sq_1._sq_1_key, _sq_1._sq_1_corr_0] [probe: 1, a.Country]
...
ExecutionPlan [compiled]
  ...
  Body
    ...
    CreateKeySet [_sq_1Keys: ValueTuple<int, string>]
    ChunkedForEach [b in cte0_bRows]
      KeySetAdd [_sq_1Keys += (1, b.Country)]
    PhaseBoundary [Select]
    ChunkedForEach [a in aRows]
      KeySetProbe [_sq_1Keys[(1, a.Country)]] [match: _sq_1KeysHasMatch]
        Assign [_sq_1KeysHasMatch = TRUE]
      KeySetProbeNoMatch
...
// === SyntaxTree:  ===
namespace GeneratedSample_Q139_CorrelatedNotExistsSubquery
{
    using System;
    ...
    public sealed class CompiledQuery : BaseOperations, ITableRunnable, IQueryProgressSource, IParameterizedRunnable
    {
        ...
        private static readonly IReadOnlyCollection<ISchemaColumn> __schemaColumns_compiled_a_0 = Array.AsReadOnly(new ISchemaColumn[] { new Column("City", typeof(string), 0), new Column("Country", typeof(string), 1) });
        private static readonly IReadOnlyCollection<ISchemaColumn> __schemaColumns_compiled_b_1 = Array.AsReadOnly(new ISchemaColumn[] { new Column("Country", typeof(string), 0) });
                    ...
                    if (aChunk is global::Musoq.Schema.DataSources.RowChunk<Musoq.Evaluator.Tests.Schema.Basic.BasicEntity> aChunkView)
                    {
                        if (aChunkView.Source is Musoq.Evaluator.Tests.Schema.Basic.BasicEntity[] aChunkViewArray)
                        {
                            ...
                            for (int aIndex = 0, aIndexCount = aChunkView.Count; aIndex < aIndexCount; ++aIndex)
                            {
                                ...
                                bool _sq_1KeysHasMatch = false;
                                var key0 = 1;
                                var key1 = a.Country;
                                var key = (key0, key1);
                                if (key1 != null && _sq_1Keys.Contains(key))
                                {
                                    _sq_1KeysHasMatch = true;
                                }

                                if (!_sq_1KeysHasMatch)
                                {
