// === Parsed Query ===
/*
select m.Value as Value from #apply.items() i cross apply i.Numbers n cross apply i.Numbers m where i.Name = 'left' and n.Value = 1 and m.Value = 2
*/
...
MultiStatement
  Project [m.Value as Value]
    Filter [(((i.Name = 'left') AND (n.Value = 1)) AND (m.Value = 2))]
      Apply [Cross]
        Apply [Cross]
...
PhysicalMultiStatement
  PhysicalProject [m.Value as Value]
    PhysicalFilter [(((i.Name = 'left') AND (n.Value = 1)) AND (m.Value = 2))]
    ...
    ChunkedForEach [i in iRows]
      ...
      ChunkedForEach [n in nRows]
        ...
        ChunkedForEach [m in mRows]
          Let [value: int = m.Value]
          If [(value = 2)]
            AppendShape [result <- ResultShape0(Value: value)]
...
// === SyntaxTree:  ===
namespace GeneratedSample_Q245_ChainedCrossApplyResidualPredicate
{
    using System;
                                                    ...
                                                    if (mChunk is global::Musoq.Schema.DataSources.RowChunk<int> mChunkView)
                                                    {
                                                        if (mChunkView.Source is int[] mChunkViewArray)
                                                        {
                                                            ...
                                                            for (int mIndex = 0, mIndexCount = mChunkView.Count; mIndex < mIndexCount; ++mIndex)
                                                            {
                                                                ...
                                                                if ((value == 2))
                                                                {
                                                                    __musoqFinalShapeRows.Add(new ResultShape0(value));
                                                                }
                                                            }
