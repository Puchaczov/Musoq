...
namespace GeneratedSample_Q30_GroupBySkipTake
{
    ...
    public sealed class CompiledQuery : BaseOperations, ITableRunnable, IQueryProgressSource, IParameterizedRunnable
    {
        ...
        {
            return QueryRows.DeferredTable<ResultRow0>("resultSliced", __columns_compiled_result_1, (queryToken) => ComputeRows_compiled_0(Provider, SourceRuntimeSettingsBySourceContextId, SourceExecutionPlans, Logger, queryToken), token);
        }
        ...
        private IEnumerable<ResultShape0> ComputeShapeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            try
            {
                ...
                foreach (var finalGroup in groupsToFinalize)
                {
                    ...
                    result.Add(new ResultShape0(finalGroup.__key0, finalGroup.__agg0.Count));
                }

                var resultSlicedRows = result.Skip(1).Take(3);
                foreach (var resultSlicedRowsRow in resultSlicedRows)
                {
                    __musoqFinalShapeRows.Add(resultSlicedRowsRow);
                }
