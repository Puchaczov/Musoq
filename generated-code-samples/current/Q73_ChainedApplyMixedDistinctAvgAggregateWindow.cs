...
MultiStatement
  Sort [WindowRef(0)]
    Project [i.Name as Name, AggRef(inm.Avg(n.Value)) as RepeatedAvg, AggRef(inm.Avg(distinct n.Value)) as DistinctAvg, WindowRef(0) as MixedAvgRowNo]
      Window [RowNumber(idx:0; order: AggRef(inm.Avg(distinct n.Value)) DESC, AggRef(inm.Avg(n.Value)) DESC, i.Name)]
        Aggregate [keys: i.Name] [aggs: RepeatedAvg, DistinctAvg]
          Apply [Cross]
            Apply [Cross]
...
PhysicalMultiStatement
  PhysicalSort [WindowRef(0)]
    PhysicalProject [i.Name as Name, AggRef(inm.Avg(n.Value)) as RepeatedAvg, AggRef(inm.Avg(distinct n.Value)) as DistinctAvg, WindowRef(0) as MixedAvgRowNo]
      PhysicalWindow [RowNumber(idx:0; order: AggRef(inm.Avg(distinct n.Value)) DESC, AggRef(inm.Avg(n.Value)) DESC, i.Name)]
        PhysicalMaterialize
          PhysicalSingleKeyAggregate [key: i.Name (String)] [aggs: RepeatedAvg, DistinctAvg]
...
ExecutionPlan [compiled]
  Shapes
    ...
    SourceEntity [m: int]
      Value: int <- direct scalar value
    AggregateGroup [WindowSourceTableAggregateGroup; keys: 1; typed aggs: 2]
    Generated [WindowSourceRow0]
      i.Name: string <- field i_Name
      RepeatedAvg: int? <- field RepeatedAvg
      DistinctAvg: int? <- field DistinctAvg
    ...
    Generated [ResultRow0]
      ...
      RepeatedAvg: int? <- field RepeatedAvg
      DistinctAvg: int? <- field DistinctAvg
      MixedAvgRowNo: long <- field MixedAvgRowNo
  ...
  Body
    ...
    ComputeRowNumberWindow [resultRowNumbers <- resultWindowRows order by windowSource.DistinctAvg DESC, windowSource.RepeatedAvg DESC, windowSource.i.Name ASC]
    CreateShapeRows [result: ResultShape0 from ResultRow0]
    ForEachIndexed [windowIndex, windowSource in resultWindowRows]
      AppendShape [result <- ResultShape0(Name: windowSource.i.Name, RepeatedAvg: windowSource.RepeatedAvg, DistinctAvg: windowSource.DistinctAvg, MixedAvgRowNo: resultRowNumbers[windowIndex])]
    SortShapeRows [result -> resultSorted by MixedAvgRowNo ASC]
...
// === SyntaxTree:  ===
namespace GeneratedSample_Q73_ChainedApplyMixedDistinctAvgAggregateWindow
{
    using System;
    ...
    public sealed class CompiledQuery : BaseOperations, ITableRunnable, IParameterizedRunnable
    {
        private static readonly Column[] __columns_compiled_result_2 = new Column[]
        {
            ...
            new Column("RepeatedAvg", typeof(int?), 1),
            new Column("DistinctAvg", typeof(int?), 2),
            new Column("MixedAvgRowNo", typeof(long), 3)
        };
        ...
        private IEnumerable<ResultRow0> ComputeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            {
                yield return new ResultRow0(__musoqShapeRow.Name, __musoqShapeRow.RepeatedAvg, __musoqShapeRow.DistinctAvg, __musoqShapeRow.MixedAvgRowNo);
            }
        }
        ...
        private IEnumerable<ResultShape0> ComputeShapeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            try
            {
                ...
                for (int windowIndex = 0; windowIndex < resultWindowRows.Count; ++windowIndex)
                {
                    ...
                    result.Add(new ResultShape0(windowSource.i_Name, windowSource.RepeatedAvg, windowSource.DistinctAvg, (long)resultRowNumbers[windowIndex]));
                }

                var resultSortedRows = result.OrderBy(static __musoqOrderRow => __musoqOrderRow, Comparer<ResultShape0>.Create((left, right) =>
                {
                    var comparison = left.MixedAvgRowNo.CompareTo(right.MixedAvgRowNo);
                    if (comparison != 0)
                        return comparison;
                    return 0;
                }));
        ...
        private static void ExtractResultRowNumbersWindowKeys(IReadOnlyList<WindowSourceRow0> resultWindowRows, WindowResultRowNumbersOrderKeysKey[] resultRowNumbersOrderKeys)
        {
            ...
            {
                WindowSourceRow0 windowSource = resultWindowRows[windowIndex];
                resultRowNumbersOrderKeys[windowIndex] = new WindowResultRowNumbersOrderKeysKey(windowSource.DistinctAvg, windowSource.RepeatedAvg, windowSource.i_Name);
            }
        }
        ...
        private static void FinalizeWindowSourceTableSingleKeyGroups(List<WindowSourceRow0> windowSourceTable, List<WindowSourceTableAggregateGroup> groupsToFinalize, CancellationToken token)
        {
            ...
            {
                token.ThrowIfCancellationRequested();
                windowSourceTable.Add(new WindowSourceRow0(finalGroup.__key0, finalGroup.__agg0.HasValue ? (int?)finalGroup.__agg0.Sum / int.CreateChecked(finalGroup.__agg0.Count) : null, Musoq.Plugins.AvgDistinctAggregateKernel<int>.Get(in finalGroup.__agg1)));
            }
        }
        ...
        private sealed class ResultRow0 : Row
        {
            ...
            {
                Name = __value0;
                RepeatedAvg = __value1;
                DistinctAvg = __value2;
                MixedAvgRowNo = __value3;
            }

            public override int Count => 4;
            public int? DistinctAvg { get; private set; }
            public long MixedAvgRowNo { get; private set; }
            public string Name { get; private set; }
            public int? RepeatedAvg { get; private set; }
            ...
            public override void AssignValue(int columnNumber, object value)
            {
                switch (columnNumber)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        RepeatedAvg = (int?)value;
                        break;
                    case 2:
                        DistinctAvg = (int?)value;
                        break;
                    case 3:
                        MixedAvgRowNo = (long)value;
                        break;
                    default:
            ...
            {
                0 => (object)Name,
                1 => (object)RepeatedAvg,
                2 => (object)DistinctAvg,
                3 => (object)MixedAvgRowNo,
                _ => throw new IndexOutOfRangeException()
            };
            public override object this[string name] => name switch
            {
                "Name" => (object)Name,
                "RepeatedAvg" => (object)RepeatedAvg,
                "DistinctAvg" => (object)DistinctAvg,
                "MixedAvgRowNo" => (object)MixedAvgRowNo,
        ...
        {
            public ResultShape0(string Name, int? RepeatedAvg, int? DistinctAvg, long MixedAvgRowNo)
            {
                this.Name = Name;
                this.RepeatedAvg = RepeatedAvg;
                this.DistinctAvg = DistinctAvg;
                this.MixedAvgRowNo = MixedAvgRowNo;
            }

            public int? DistinctAvg { get; }
            public long MixedAvgRowNo { get; }
            public string Name { get; }
            public int? RepeatedAvg { get; }
        }
        ...
        private readonly struct WindowResultRowNumbersOrderKeysKey : System.IEquatable<WindowResultRowNumbersOrderKeysKey>, System.IComparable<WindowResultRowNumbersOrderKeysKey>
        {
            ...
            public WindowResultRowNumbersOrderKeysKey(int? value0, int? value1, string value2)
            {
                _value0 = value0;
                _value1 = value1;
                _value2 = value2;
            ...
            {
                return System.Collections.Generic.EqualityComparer<int?>.Default.Equals(_value0, other._value0) && System.Collections.Generic.EqualityComparer<int?>.Default.Equals(_value1, other._value1) && System.String.Equals(_value2, other._value2, System.StringComparison.Ordinal);
            }
            ...
            public override int GetHashCode()
            {
                ...
                hash.Add(_value0);
                hash.Add(_value1);
                hash.Add(_value2, System.StringComparer.Ordinal);
            ...
            private static int CompareValue2(string left, string right)
            {
                if (left == null)
        ...
        {
            public WindowSourceRow0(string __value0, int? __value1, int? __value2)
            {
                i_Name = __value0;
                RepeatedAvg = __value1;
                DistinctAvg = __value2;
        ...
        {
            public Musoq.Plugins.AvgAggregateKernel<int>.State __agg0;
            public Musoq.Plugins.AvgDistinctAggregateKernel<int>.State __agg1;
            ...
            {
                Musoq.Plugins.AvgAggregateKernel<int>.Merge(ref this.__agg0, in source.__agg0);
                Musoq.Plugins.AvgDistinctAggregateKernel<int>.Merge(ref this.__agg1, in source.__agg1);
            }
        }
    }
}
