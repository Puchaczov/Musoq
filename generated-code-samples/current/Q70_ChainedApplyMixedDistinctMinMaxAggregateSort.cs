...
namespace GeneratedSample_Q70_ChainedApplyMixedDistinctMinMaxAggregateSort
{
    ...
    public sealed class CompiledQuery : BaseOperations, ITableRunnable, IQueryProgressSource, IParameterizedRunnable
    {
        ...
        private IEnumerable<ResultRow0> ComputeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            {
                yield return new ResultRow0(__musoqShapeRow.Name, __musoqShapeRow.RepeatedMin, __musoqShapeRow.DistinctMin, __musoqShapeRow.RepeatedMax, __musoqShapeRow.DistinctMax);
            }
        }
        ...
        private IEnumerable<ResultShape0> ComputeShapeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            try
            {
                ...
                {
                    token.ThrowIfCancellationRequested();
                    result.Add(new ResultShape0(finalGroup.__key0, finalGroup.__agg0.HasValue ? (int?)finalGroup.__agg0.Value : null, Musoq.Plugins.MinDistinctAggregateKernel<int>.Get(in finalGroup.__agg1), finalGroup.__agg2.HasValue ? (int?)finalGroup.__agg2.Value : null, Musoq.Plugins.MaxDistinctAggregateKernel<int>.Get(in finalGroup.__agg3)));
                }

                var resultSortedRows = result.OrderBy(static __musoqOrderRow => __musoqOrderRow, Comparer<ResultShape0>.Create((left, right) =>
                {
                    var comparison = Nullable.Compare(left.DistinctMax, right.DistinctMax);
                    comparison = -comparison;
                    if (comparison != 0)
                        return comparison;
                    comparison = Nullable.Compare(left.RepeatedMax, right.RepeatedMax);
                    comparison = -comparison;
                    if (comparison != 0)
                        return comparison;
                    comparison = Nullable.Compare(left.DistinctMin, right.DistinctMin);
                    if (comparison != 0)
                        return comparison;
                    comparison = Nullable.Compare(left.RepeatedMin, right.RepeatedMin);
                    if (comparison != 0)
                        return comparison;
        ...
        {
            public ResultRow0(string __value0, int? __value1, int? __value2, int? __value3, int? __value4)
            {
                Name = __value0;
        ...
        {
            public ResultShape0(string Name, int? RepeatedMin, int? DistinctMin, int? RepeatedMax, int? DistinctMax)
            {
                this.Name = Name;
