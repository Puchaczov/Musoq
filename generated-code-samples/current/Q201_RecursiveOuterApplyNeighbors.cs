// === Parsed Query ===
/*
with recursive reachable (Id, Depth) as (select RootId, 0 from #graph.roots() union (Id) select (case when e.TargetId is null then r.Id else e.TargetId end), r.Depth + 1 from reachable r outer apply #graph.neighbors(r.Id) e) select Id, Depth from reachable order by Id
*/
...
// === SyntaxTree:  ===
namespace GeneratedSample_Q201_RecursiveOuterApplyNeighbors
{
    using System;
    ...
    public sealed class CompiledQuery : BaseOperations, ITableRunnable, IParameterizedRunnable
    {
        ...
        private static readonly IReadOnlyCollection<ISchemaColumn> __schemaColumns_compiled_e_1 = Array.AsReadOnly(new ISchemaColumn[] { new Column("TargetId", typeof(int), 1) });
            ...
            try
            {
                ...
                while (cte0CurrentFrontier.Count > 0)
                {
                    ...
                    for (int cte0CurrentFrontierIndex = 0; cte0CurrentFrontierIndex < cte0CurrentFrontier.Count; ++cte0CurrentFrontierIndex)
                    {
                        ...
                        var cte0NextFrontier_eRows = cte0NextFrontier_eRowsSource.Chunks;
