// === Parsed Query ===
/*
with recursive counter (Value) as (select 1 from values {{ Seed: 1 }} seed union all select c.Value + 1 from counter c where c.Value < 3) select Value from counter order by Value
*/
...
ExecutionPlan [compiled]
  ...
  Body
    RecursiveCte [counter; result cte0; frontiers cte0CurrentFrontier, cte0NextFrontier; identity none; max iterations 7; max rows 25; max snapshot rows 10000000]
      Anchor
        CreateValuesRows [cte0CurrentFrontier_seedRows: seedValuesD6F9BDFERow0 x 1]
        ForEach [seed in cte0CurrentFrontier_seedRows]
          RecursiveAppend [cte0CurrentFrontier <- Cte0Row0(Value: 1); identity none; guard cte0.Count + cte0CurrentFrontier.Count < 25]
      RecursiveMember
        ForEach [c in cte0CurrentFrontier]
          If [(c.Value < 3)]
            RecursiveAppend [cte0NextFrontier <- Cte0Row0(Value: (c.Value + 1)); identity none; guard cte0.Count + cte0NextFrontier.Count < 25]
...
// === SyntaxTree:  ===
namespace GeneratedSample_Q220_RecursiveLimitOverrideCodeShape
{
    using System;
        ...
        private IEnumerable<ResultShape0> ComputeShapeRows_compiled_0(ISchemaProvider provider, IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> sourceRuntimeSettingsBySourceContextId, IReadOnlyDictionary<string, SourceExecutionPlan> sourceExecutionPlans, ILogger logger, CancellationToken token)
        {
            ...
            try
            {
                ...
                foreach (var seed in cte0CurrentFrontier_seedRows)
                {
                    ...
                    if (cte0.Count + cte0CurrentFrontier.Count >= 25)
                    {
                        throw new global::Musoq.Evaluator.Exceptions.RecursiveCteLimitExceededException("counter", global::Musoq.Parser.Diagnostics.DiagnosticCode.MQ7008_RecursiveCteRowLimitExceeded, 25);
                    }

                    cte0CurrentFrontier.Add(new Cte0Row0(1));
                }
                ...
                while (cte0CurrentFrontier.Count > 0)
                {
                    ...
                    if (__cte0Iteration >= 7)
                    {
                        throw new global::Musoq.Evaluator.Exceptions.RecursiveCteLimitExceededException("counter", global::Musoq.Parser.Diagnostics.DiagnosticCode.MQ7007_RecursiveCteIterationLimitExceeded, 7);
                    }
                    ...
                    for (int cte0CurrentFrontierIndex = 0; cte0CurrentFrontierIndex < cte0CurrentFrontier.Count; ++cte0CurrentFrontierIndex)
                    {
                        ...
                        if ((c.Value < 3))
                        {
                            ...
                            if (cte0.Count + cte0NextFrontier.Count >= 25)
                            {
                                throw new global::Musoq.Evaluator.Exceptions.RecursiveCteLimitExceededException("counter", global::Musoq.Parser.Diagnostics.DiagnosticCode.MQ7008_RecursiveCteRowLimitExceeded, 25);
                            }
