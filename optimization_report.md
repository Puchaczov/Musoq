...
### Step 7: Non-Equi Join Optimization (Sort-Merge / Early Break)
**Goal**: Improve performance of non-equi joins (e.g., `A.Val > B.Val`) which cannot use Hash Join.
**Strategy**:
*   Materialize and sort the inner table based on the join key.
*   Iterate the outer table and the sorted inner table.
*   Terminate the inner loop early when the condition is no longer met (e.g., for `>` condition, stop once we reach a value that is not smaller).
*   **Critical Optimization**: Replaced `List<dynamic>` with `List<IObjectResolver>` and used static casting in generated code to avoid `RuntimeBinderException` and dynamic dispatch overhead.
**Status**: Implemented and verified.
**Benchmarks**:
*   **Scenario**: 3-way Join (`A.Pop > B.Pop > C.Pop`) with 100 rows each.
    *   Result Size: ~166,000 rows.
    *   Execution Time: **161.5 ms**
    *   Throughput: **~1,000,000 rows/sec**
*   **Scenario**: 3-way Join with 200 rows each.
    *   Result Size: ~1,333,000 rows.
    *   Execution Time: **1,207.3 ms**
    *   Throughput: **~1,100,000 rows/sec**
**Impact**: Enables efficient processing of non-equi joins that would otherwise require a full Cartesian product scan ($O(N^2)$) without early termination. The sorting allows reducing the search space significantly.
...
### Step 7: Implement Full Sort-Merge Join for Non-Equi Joins (Completed)
**Goal**: Optimize non-equi joins (e.g., `A.Id > B.Id`) using a true Sort-Merge strategy with a sliding window.
**Strategy**:
*   Implement a **Full Sort-Merge Join** logic.
*   **Algorithm**:
    1.  Materialize and **Sort** the Inner relation (B).
    2.  Materialize and **Sort** the Outer relation (A).
    3.  Use a **Sliding Window** (Two-Pointer) approach.
        *   Maintain a `limit` pointer in B.
        *   For each row in A, advance `limit` while the condition holds (e.g., `B[limit] < A`).
        *   Emit all rows in B from `0` to `limit` (for `>`) or `limit` to `end` (for `<`).
    4.  **Optimization**: The inner loop (emission) runs without any conditional checks (`if`), effectively performing a "bulk copy" of the matching range.
**Status**: Implemented and verified.
**Benchmarks (2000 Rows)**:
*   **Nested Loop**: ~1458 ms (1408 MB)
*   **Full Sort-Merge Join**: **1313.0 ms** (1164 MB)
*   **Speedup**: **~10%** faster than Nested Loop.
*   **Memory**: **~17%** reduction in memory usage.
**Analysis**:
*   Sorting both sides allows us to skip re-scanning the prefix of B that we already know matches.
*   The "bulk copy" loop (unchecked iteration) reduces CPU branch mispredictions.
*   Materializing A adds some memory overhead initially, but the overall efficiency gains (and reduced intermediate allocations) result in a net memory decrease.

**Generated Code Snippet (Full Sort-Merge)**:
```csharp
// 1. Materialize & Sort Inner
var innerList = new List<Row>(); ...; innerList.Sort(...);

// 2. Materialize & Sort Outer
var outerList = new List<Row>(); ...; outerList.Sort(...);

// 3. Two-Pointer Iteration
var limit = 0;
foreach (var outerRow in outerList)
{
    var outerKey = ...;
    
    // Advance limit (Sliding Window)
    while (limit < innerList.Count) {
        var innerRow = innerList[limit];
        if (!Condition(outerKey, innerRow)) break;
        limit++;
    }
    
    // Bulk Emit (No checks!)
    for (int i = 0; i < limit; i++) {
        var innerRow = innerList[i];
